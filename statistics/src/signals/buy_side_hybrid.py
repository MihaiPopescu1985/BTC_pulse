...
from src.signals.buy_side_common import (
    DISTANCE_COLUMN,
    DOWN_DIRECTION,
    SWING_DIRECTION_COLUMN,
    SWING_ID_COLUMN,
    build_retained_buy_source,
)
from src.signals.reversal_zone_models import (
    DEFAULT_BUY_STRICT_TARGET,
    DEFAULT_BUY_TARGET,
)
from src.signals.swing_extreme_timing import THRESHOLDS, TOP_BUCKETS, clip01
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Optional prebuilt exploration surface. If absent, rebuild promoted buy-side source directly from reversal-zone data.",
    )
    parser.add_argument(
...
def load_or_build_source(
    exploration_scores_csv: str | Path,
    reversal_zone_dataset_csv: str | Path,
) -> tuple[pd.DataFrame, str]:
    exploration_path = Path(exploration_scores_csv)
    if exploration_path.exists():
        return load_scores(exploration_path), "prebuilt_exploration_surface"
    scored, _phase_features = build_retained_buy_source(reversal_zone_dataset_csv)
    for column in HYBRID_FEATURES:
        scored[column] = pd.to_numeric(scored[column], errors="coerce")
    return scored.sort_values("date").reset_index(drop=True), "rebuilt_from_reversal_zone_dataset"
...
def make_recommendation(comparison: pd.DataFrame) -> tuple[str, list[str]]:
    ...
    candidate_top10_zone5 = metric_lookup(candidate, comparison, "top_bucket", "top_10pct", "zone_5_hit_rate") if False else np.nan
    candidate_top10_zone5 = metric_lookup(
        comparison,
        candidate,
        "top_bucket",
        "top_10pct",
    ...
    fixed_top10_zone5 = metric_lookup(
        comparison,
        "baseline_fixed_weight",
        "top_bucket",
        "top_10pct",
    ...
    notes = [
        f"Best hybrid by average best-picked distance: `{candidate}`.",
        (
            f"Fixed baseline avg best distance `{fixed_distance:.3f}` vs candidate `{candidate_distance:.3f}`; "
            f"5% hit `{fixed_zone5:.3f}` vs `{candidate_zone5:.3f}`, "
            f"3% hit `{fixed_zone3:.3f}` vs `{candidate_zone3:.3f}`."
        ),
    ]

    if (
        np.isfinite(candidate_distance)
        and np.isfinite(fixed_distance)
        and candidate_distance < fixed_distance
        and candidate_zone5 >= fixed_zone5
        and candidate_top10_zone5 >= fixed_top10_zone5
    ):
        return "Continue", notes + ["Hybrid materially improves best-pick proximity without giving back top-decile quality."]

    if np.isfinite(candidate_distance) and np.isfinite(fixed_distance) and candidate_distance < fixed_distance:
        return "Continue cautiously", notes + ["Hybrid improves one core dimension, but the gain is not dominant across all reference checks."]

    return "Pause / likely dead end", notes + ["No hybrid materially beats the fixed baseline in a convincing way."]
...
def render_markdown(
    ...
    source_note = (
        "`out/swing_bottom/buy_side_exploration_scores.csv`"
        if source_mode == "prebuilt_exploration_surface"
        else "direct rebuild from `out/swing_bottom/reversal_zone_dataset.csv` via retained signal-owned helpers"
    )
...
def main() -> None:
    args = parse_args()
    source, source_mode = load_or_build_source(
        exploration_scores_csv=args.buy_side_exploration_scores_csv,
        reversal_zone_dataset_csv=args.reversal_zone_dataset_csv,
    )
    ...
    out_md.write_text(render_markdown(scored, comparison, coefficients, source_mode), encoding="utf-8")
