...
from src.models.first_touch_classifier import (
    FIRST_TOUCH_LABELS,
...
DEFAULT_OUT_DIR = OUT_DIR / "first_touch_classifier_eval"
DEFAULT_MODEL_PATH = OUT_DIR / "first_touch_classifier" / "model.joblib"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and walk-forward evaluate a supervised SAFE first-touch classifier.",
    )
    ...
    parser.add_argument("--days", type=int, default=2, help="Forward horizon in days. Default: 2")
    parser.add_argument("--up-pct", type=float, default=0.02)
    parser.add_argument("--down-pct", type=float, default=0.02)
    parser.add_argument(
        "--ambiguity-mode",
        choices=["pessimistic", "optimistic", "skip_ambiguous"],
...
def main() -> None:
    ...
    dataset = build_first_touch_classifier_dataset(
        args.price_json,
        args.features_json,
    ...
    for fold in folds:
        ...
        pred_top2 = pred_frame.apply(lambda row: "|".join(row.nlargest(min(2, len(row))).index.tolist()), axis=1)
        ...
        for idx in X_test.index:
            ...
            for class_name in set(FIRST_TOUCH_LABELS) | set(classes):
    ...
    majority_accuracy = float(np.mean(labels == majority_class))
    ...
    print("=== SAFE First-Touch Classifier ===")
